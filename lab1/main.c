...
//set the pins to general-purpose output mode.
GPIOC->MODER = (1<<12) | (1<<14);
GPIOA->MODER = 0;
//PUPDR register, set PC8 and PC9 to have no pull-up/down resistors.
GPIOC->PUPDR = (1<<13) | (1<<15);
GPIOA->PUPDR = 2;
GPIOC->OSPEEDR = 0;
...
if (GPIOA->IDR & 1) { //signal is set/high
